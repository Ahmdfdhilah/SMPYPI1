...
@section('content')
    ...
    <main class="w-full bg-white pt-12 pb-48 px-8 md:px-11 lg:px-24">
        ...
        <section class="min-h-screen py-24 lg:flex" data-aos="fade-up" data-aos-delay="200">
            <div class="flex flex-col justify-center w-full p-8 bg-cover lg:px-12 xl:px-32 lg:w-1/2"
                style="background-image: url('/frontend/contact.jpg');">
            ...
            <div class="flex flex-col bg-white justify-center w-full p-8 lg:w-1/2 lg:px-12 xl:px-24 ">
                <form action="{{ route('contact.submit') }}" method="post">
                    ...
                    <div class="grid gap-6 mb-6 md:grid-cols-2">
                        ...
                        <div>
                            <label for="type" class="block mb-2 text-sm font-medium">Tipe</label>
                            <select name="type" id="type" value="{{ old('type') }}"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                placeholder="Saran atau aduan" required>
                                <option value="saran">Saran</option>
                                <option value="saab">Aduan</option>
                            </select>
                            @error('type')
                                <p class="text-red-500 text-sm">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="tel" id="phone" name="phone" value="{{ old('phone') }}"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                placeholder="08XXXXXXXXXX" pattern="08[0-9]{10}" required>
                            @error('phone')
                                <p class="text-red-500 text-sm">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="mb-6">
                        <label for="large-input" class="block mb-2 text-sm font-medium">Isi Pesan</label>
                        <textarea id="large-input" name="message"
                            class="block w-full p-4 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 sm:text-md focus:ring-blue-500 focus:border-blue-500">{{ old('message') }}</textarea>
                        @error('message')
                            <p class="text-red-500 text-sm">{{ $message }}</p>
                        @enderror
                    </div>
